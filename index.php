...
function fix_css($url) {
    ...
    return $delim.preg_replace('#([\(\),\s\'"\\\])#', '\\$1', encode_resource_url(trim
                                (preg_replace('#\\\(.)#', '$1', trim($url, $delim))))).$delim;
...
if (!$data_url['host']) {
 ...
 if ($data_url['scheme'] == 'http') {
     $fp = fsockopen("tsp://$data_url[host]", 80);
 } else {
     $fp = fsockopen("ssl://$data_url[host]", 443);
 }

 if (!$fp) {
     exit('Server is not responding!');
 }
